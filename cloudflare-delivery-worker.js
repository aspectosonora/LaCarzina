...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (!env.GOOGLE_MAPS_API_KEY) return json({ ok: false, text: 'Por confirmar segun ubicacion' });
      ...
      if (!element || element.status !== 'OK' || !element.distance?.value) {
        return json(confirmResponse(element?.status || maps.status || 'google-no-distance'));
      }
...
function confirmResponse(reason) {
  return {
    ok: false,
    reason,
    price: null,
